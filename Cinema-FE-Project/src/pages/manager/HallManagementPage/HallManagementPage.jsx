...
const SeatEditor = ({ hall, onClose }) => {
  ...
  useEffect(() => {
    const fetchLayout = async () => {
      ...
      try {
        setLoading(true);
        const res = await managerService.getSeatLayout(hallId);
        ...
        if (Array.isArray(payload?.matrix)) {
          payload.matrix.forEach((rowItem) => {
            maxR = Math.max(maxR, Number(rowItem.row || 0));
            (rowItem.seats || []).forEach((seat) => {
              maxC = Math.max(maxC, Number(seat.col || 0));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(err?.response?.data?.message || err?.message || "Lỗi khi lưu sơ đồ ghế");
    } finally {
      setSaving(false);
    }
  };
  ...
  const getSeatColor = (type) => {
    switch (type) {
      ...
      case "VIP":
        return "bg-amber-500 border-amber-400 text-white shadow-lg shadow-amber-500/20";
      case "COUPLE":
        return "bg-pink-600 border-pink-500 text-white shadow-lg shadow-pink-600/20";
      default:
        return "bg-transparent border-white/5 text-gray-800";
    }
  };
  ...
  return (
    ...
    >
      <div className="p-8 border-b border-white/10 flex justify-between items-center bg-white/[0.02]">
        <div>
          <h3 className="text-2xl font-bold">{hall?.name || "Hall"} - Sơ đồ ghế</h3>
          <div className="flex items-center gap-4 mt-1">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-12 bg-white/5 border border-white/10 rounded px-1 text-xs text-white"
              />
            </div>
            <div className="flex items-center gap-2">
        ...
        {loading ? (
          ...
          <>
            <div className="w-full max-w-2xl h-8 bg-gradient-to-b from-white/20 to-transparent rounded-lg mb-20 relative">
              <p className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[10px] uppercase font-black tracking-[0.5em] text-gray-500">
                Màn Hình
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 min-w-max pb-10">
              {Array.from({ length: rows }).map((_, r) => (
                <div key={r} className="flex gap-3 items-center">
                    ...
                    {Array.from({ length: cols }).map((_, c) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-8 h-8 rounded-lg border transition-all duration-200 flex items-center justify-center group ${getSeatColor(
                            type
                          )}`}
                        >
                  ...
                  <span className="w-6 text-xs font-bold text-gray-600 uppercase text-right">
            ...
            {[
              ...
              <button
                ...
                className={`flex items-center gap-3 px-4 py-2 rounded-xl border transition-all ${
                  selectedType === t.id ? "bg-white/10 border-red-600" : "bg-transparent border-transparent"
                }`}
              >
  ...
  >
    ...
    <div className="flex justify-between items-start mb-6 relative z-10">
      ...
      <div className="flex gap-1">
        ...
        <button
          onClick={() => onDelete(getHallId(hall))}
    ...
    <h3 className="text-2xl font-black mb-1 font-display tracking-tight text-white">{hall?.name}</h3>
      ...
      <div className="flex justify-between items-end bg-white/5 p-4 rounded-2xl border border-white/5">
        <div>
          ...
          <p className="text-2xl font-black text-white">
            {hall?.capacity} <span className="text-xs font-medium text-gray-500 ml-1">Seats</span>
          </p>
        </div>
      </div>
    </div>
...
export default function HallManagementPage() {
  ...
  const normalizedHalls = useMemo(() => {
    return (Array.isArray(halls) ? halls : []).map((h) => ({
      ...h,
      _resolvedId: getHallId(h),
    }));
  }, [halls]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error(err);
      alert(err?.response?.data?.message || err?.message || "Delete hall failed");
    }
  };
  ...
  const handleSave = async (data) => {
    try {
      ...
      console.error(err);
      alert(err?.response?.data?.message || err?.message || "Save hall failed");
    }
  };
    ...
    <div className="space-y-8 pb-10 overflow-x-hidden">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            Manage physical cinema rooms and configure interactive seat layouts.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {normalizedHalls.map((hall) => (
            <HallCard
              key={hall._resolvedId || hall.name}
              hall={hall}
